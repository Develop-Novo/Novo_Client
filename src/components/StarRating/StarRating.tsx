...
function StarRating({myRating}:StarRatingProps){
    ...
    const [clickedItem, setClickedItem] = useState<number | null>(myRating ? myRating.star : 0);
    ...
    const handleClick = (itemId: number) => {
        const postNewStar = async(star:number) => {
            try {
				const response = await axios.post('http://35.216.73.185:8080/star/new', {
                    memberId: 1,
                    contentId: 1,
                    star: star,
                },{
        ...
        const modifyStar = async(star:number, starId: number) => {
            try {
				const response = await axios.put(`http://35.216.73.185:8080/star/id/${starId}`, {
                    memberId: 1,
                    contentId: 1,
                    star: star,
                },{
